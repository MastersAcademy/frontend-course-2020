...
window.onload = function () {
    ...
    send1Node.addEventListener('click', (event) => receiver(inputNode.value, 0));
    send2Node.addEventListener('click', (event) => receiver(inputNode.value, 1));
...
function creater(btnID, text) {
    if ('content' in document.createElement('template')) {
        const avatar_color_class = ['color-one', 'color-two'];
        const author_name = ['Tom', 'Jerry'];
